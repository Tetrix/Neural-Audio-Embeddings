...
encoder = Encoder(features_train[0][0].size(1), encoder_hidden_size, encoder_layers).to(device)
encoder_optimizer = optim.Adam(encoder.parameters(), lr=encoder_lr)

# initialize the Decoder
decoder = Decoder(encoder_hidden_size, decoder_hidden_size, decoder_layers, encoder_layers, batch_size).to(device)
...
if skip_training == False:
    ...
    checkpoint = torch.load("weights/state_dict_1.pt", map_location=torch.device("cpu"))
    encoder.load_state_dict(checkpoint["encoder"])
    decoder.load_state_dict(checkpoint["decoder"])
    encoder.eval()
    decoder.eval()
...
#get_cosine_similarity(encoder, text_data, features_dev, "mother", "mother", device)
#print("Done...")

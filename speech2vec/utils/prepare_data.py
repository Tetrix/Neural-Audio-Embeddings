        ...
        if len(feature) > 1:
            for i in range(len(feature)):
                ...
                if i == 0:
                    ...
                    #pair = (torch.FloatTensor(segment), torch.FloatTensor(next_segment), torch.FloatTensor(torch.zeros(1, 40)))
                    pass
                elif i == len(feature) - 1:
                    #prev_segment = feature[i-1].astype(np.float)
                    #pair = (torch.FloatTensor(segment), torch.FloatTensor(prev_segment), torch.FloatTensor(torch.zeros(1, 40)))
                    pass
                else:
                    prev_segment = feature[i-1].astype(np.float)
                    next_segment = feature[i+1].astype(np.float)
